...
export function CrewRecommendationCard({ crew, manualInstallerCount, className = '' }: CrewRecommendationCardProps) {
  if (!crew) {
    return (
      <div className={`rounded-xl border border-slate-200/80 bg-slate-50/80 p-4 text-sm text-slate-600 ${className}`}>
        <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Crew recommendation</p>
        <p className="mt-2">Add estimate lines to see staffing guidance.</p>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm ${className}`}>
      <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Crew recommendation</p>
      <p className="mt-1 text-xs text-slate-600">Schedule-focused; labor dollars still follow total hours × rate.</p>
      ...
      {manualInstallerCount !== crew.recommendedCrew ? (
        <p className="mt-2 text-[11px] font-medium text-slate-700">
          Manual crew in Setup: <span className="tabular-nums">{manualInstallerCount}</span> installer{manualInstallerCount === 1 ? '' : 's'}
        </p>
      ) : null}
