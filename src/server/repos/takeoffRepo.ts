...
export async function getConfiguredLaborRatePerHour(): Promise<number> {
  const row = (await dbGet('SELECT default_labor_rate_per_hour FROM settings_v1 WHERE id = ?', ['global'])) as
...
async function resolveCatalogDefaults(input: Partial<TakeoffLineRecord>): Promise<{ materialCost?: number; laborMinutes?: number }> {
  ...
  if (input.sku) {
    const row = (await dbGet(`SELECT base_material_cost, base_labor_minutes FROM ${table} WHERE lower(sku) = lower(?) LIMIT 1`, [
      input.sku,
    ])) as { base_material_cost: number; base_labor_minutes: number } | undefined;
    if (row) {
      return {
...
export async function createTakeoffLine(
  ...
  const baseLaborCost =
    input.baseLaborCost !== undefined ? Number(input.baseLaborCost) || 0 : resolveUnitLaborCostFromMinutes(laborMinutes, laborRatePerHour);
  ...
  const line: TakeoffLineRecord = {
    ...
    sourceLineType:
      input.sourceLineType ??
      (String(input.sourceType || '').toLowerCase() === 'quote_subtotal'
        ? 'quote_subtotal'
        : input.catalogItemId
          ? 'catalog_item'
          : 'manual'),
...
export async function updateTakeoffLine(lineId: string, input: Partial<TakeoffLineRecord>): Promise<TakeoffLineRecord | null> {
  const existing = await getTakeoffLine(lineId);
